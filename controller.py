        ...
        while not user_choice:
            ...
            match user_choice:
                ...
                case '3':
                    ...
                    records = model.search_record(search_choice, query, FILEPATH)
                    if records:
                        user_interface.print_message('Найдены следующие записи:')
                        user_interface.show_database(records)
                        find_id = user_inputs.get_random_input('id нужной записи')
                        if not model.check_id(find_id, FILEPATH):
                            user_interface.print_message(f'Запись c id "{find_id}" отсутствует')
                        else:
                            inner_menu = True
                            while inner_menu:
                                ...
                                match change_choice:
                                    case '1':
                                        user_interface.fields_menu('изменения', 1)
                                        field_choice = input('-: ').lower()
                                        new_value = user_inputs.get_random_input(f'новое значение поля {field_choice}')
                                        updated_record = model.get_updates(find_id, field_choice, new_value, FILEPATH)
                                        user_interface.show_record(updated_record)
                                        if user_inputs.confirm_choice('Внести эти изменения?'):
                                            model.change_field(find_id, field_choice, new_value, FILEPATH)
                                        inner_menu = False
                                    case '2':
                                        record = model.search_record('1', find_id, FILEPATH, compliance=True)
                        ...
                        user_interface.print_message(f'Записи по запросу "{query}" отсутствуют')
                case '0':
                    running = False
